...
date: "Created 26 January 2021; updated `r format(Sys.time(), '%B %d, %Y at %H:%M:%S')`"
...
This artical is developed based on [Statistical Computing](https://www.stat.cmu.edu/~ryantibs/statcomp/lectures/plotting.Rmd) by [Ryan Tibshirani](http://www.stat.cmu.edu/~ryantibs/)
...
set.seed(0) # Give the same display when using random numbers
...
plot(x, y, pch=20) # for small Filled circles
plot(x, y, pch=".") # for single pixels
```
...
plot(x, y, pch=19, col="green") # Green
```
...
```{r}
biden.lines =
  readLines("D:/R_Images/Biden_Speech.txt")
biden.words = strsplit(paste(biden.lines, collapse=" "),
                       split="[[:space:]]|[[:punct:]]")[[1]]
biden.words = tolower(biden.words[biden.words != ""])
biden.wlens = nchar(biden.words) # Words length
hist(biden.wlens) # Histogram
...
```{r}
hist(biden.wlens, col="pink", freq=TRUE) # Frequency scale, default
hist(biden.wlens, col="pink", freq=FALSE, # Probability scale, and more options
     breaks=0:20, xlab="Word length", main="Biden word lengths") 
```
...
```{r}
hist(biden.wlens, col="pink", freq=FALSE, breaks=0:20, 
     xlab="Word length", main="Biden word lengths")
...
```{r}
density.est = density(biden.wlens, adjust=1.5) # 1.5 times the default bandwidth
class(density.est)
names(density.est)
...
To draw a surface, use `surface()`, available at http://www.stat.cmu.edu/~ryantibs/statcomp/lectures/surface.R. (This is a function written by Professor Tibs, relying on the built-in `persp()` function)
...
```{r}
persp.mat = surface(x^3 + y^3, from.x=-3, to.x=3, 
                    from.y=-3, to.y=3,theta=25, phi=15,
